...
def generate_story(world, parts, nodes_per_part):
    ...
    past_templates = [
        ...
        "تكتشف سيفاً صدئاً مغروزة في حجر، وحوله نقوش تحذر من لمسه.\n\nالسيف يبدو عادياً للوهلة الأولى، ولكنك تشعر بقوة غامضة تنبعث منه، تجذبك نحوه كالمغناطيس. النقوش المحيطة به مكتوبة بلغة قديمة جداً، ويبدو أنها تعويذة حماية قوية.\n\nهل تتجاهل التحذيرات وتحاول سحب السيف، أم تحاول قراءة التعويذة لفهم طبيعة اللعنة؟",
    ...
    future_templates = [
        ...
        "تجد شريحة بيانات متوهجة ملقاة في زقاق ضيق ومظلم.\n\nالشريحة تبدو متطورة جداً، وتصدر نبضات ضوئية خافتة كأنها قلب ينبض بالمعلومات. لا شك أن من أسقطها يبحث عنها بشدة، وأنها تحتوي على أسرار خطيرة.\n\nهل تحتفظ بالشريحة وتحاول فك تشفيرها، أم تتركها في مكانها وتتجنب التورط في مشاكل لا حصر لها؟",
    ...
    for arch_id, arch_name_ar in archetypes.items():
        ...
        for i in range(1, 21):
            arch_jobs.append({
                ...
                "title_ar": f"مهمة حصرية: دروب {arch_name_ar} {i}",
                "desc_ar": f"هذه المهمة تتطلب خبرتك كـ {arch_name_ar}. فقط من يمتلك مهاراتك الخاصة يمكنه كشف الأسرار وحل الأزمة الحالية.",
                "archetype": arch_id,
                "base_reward_gold": random.randint(100, 500),
                "base_reward_xp": random.randint(300, 1000),
                "rare_drop_chance": 0.05, # Higher rare drop chance for specialized jobs
                "rare_event": {
                    "event_text_ar": f"أثناء إنجاز المهمة، استخدمت غريزتك كـ {arch_name_ar} لاكتشاف مخبأ سري!",
                    "bonus_item": "token_of_mastery"
                }
            })
...
def generate_world_data():
    factions = [
        {"id": "f_1", "name_ar": "حراس العهد", "desc_ar": "تنظيم قديم يحمي الأسرار التاريخية العظيمة من الوقوع في الأيدي الخطأ.", "leader_name_ar": "إلياس الحكيم"},
        {"id": "f_2", "name_ar": "نقابة الظلال", "desc_ar": "مجموعة سرية من القتلة واللصوص يتحكمون في الأسواق السوداء.", "leader_name_ar": "طارق الصامت"},
        {"id": "f_3", "name_ar": "فرسان النور", "desc_ar": "نظام عسكري يحافظ على النظام العام ويحارب الفوضى بكل أشكالها.", "leader_name_ar": "سيران قائدة الجيوش"}
    ]
    save_json({"factions": factions}, "content/world/factions.json")

    npcs = [
        {"id": "npc_001", "name_ar": "التاجر المجهول", "role": "تاجر سلع نادرة", "location": "الأسواق التحتية", "dialogue_ar": "لدي بضائع لم ترها من قبل، ولكن هل تملك الثمن؟"},
        {"id": "npc_002", "name_ar": "العرافة العمياء", "role": "موجهة مهام روحية", "location": "غابة الأرواح", "dialogue_ar": "عيناي لا تريان النور، ولكني أرى مصيرك بوضوح تام..."},
        {"id": "npc_003", "name_ar": "الحداد الأسطوري", "role": "صانع أسلحة ودروع", "location": "الجبل البركاني", "dialogue_ar": "الفولاذ يجب أن يطرق وهو ساخن، تماماً كشخصيتك."}
    ]
