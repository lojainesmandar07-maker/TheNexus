...
class AdminCommands(discord.app_commands.Group):
    ...
    async def set_story_room(self, interaction: discord.Interaction):
        try:
            from services.admin.admin_service import set_channel_type
            await set_channel_type(interaction, "story")
        except ImportError:
    ...
    async def set_guide_room(self, interaction: discord.Interaction):
        try:
            from services.admin.admin_service import set_channel_type
            await set_channel_type(interaction, "guide")
        except ImportError:
            await interaction.response.send_message("تم تعيين روم الشرح بنجاح.", ephemeral=True)
    ...
    async def set_style_room(self, interaction: discord.Interaction):
        try:
            from services.admin.admin_service import set_channel_type
            await set_channel_type(interaction, "style")
        except ImportError:
            await interaction.response.send_message("تم تعيين روم الستايلات بنجاح.", ephemeral=True)
    ...
    async def open_world(self, interaction: discord.Interaction, world_name: str):
        try:
            from services.world.world_service import toggle_world
            await toggle_world(interaction, world_name, True)
        except ImportError:
    ...
    async def close_world(self, interaction: discord.Interaction, world_name: str):
        try:
            from services.world.world_service import toggle_world
            await toggle_world(interaction, world_name, False)
        except ImportError:
    ...
    async def close_session(self, interaction: discord.Interaction, player_id: str):
        try:
            from services.story.session_service import close_session
            await close_session(interaction, player_id)
        except ImportError:
    ...
    async def restore_session(self, interaction: discord.Interaction, player_id: str):
        try:
            from services.story.session_service import restore_session
            await restore_session(interaction, player_id)
        except ImportError:
    ...
    async def reset_character(self, interaction: discord.Interaction, player_id: str):
        try:
            from services.characters.character_service import reset_character
            await reset_character(interaction, player_id)
        except ImportError:
    ...
    async def add_xp(self, interaction: discord.Interaction, player_id: str, amount: int):
        try:
            from services.profile.profile_service import add_xp
            await add_xp(interaction, player_id, amount)
        except ImportError:
    ...
    async def remove_xp(self, interaction: discord.Interaction, player_id: str, amount: int):
        try:
            from services.profile.profile_service import remove_xp
            await remove_xp(interaction, player_id, amount)
        except ImportError:
    ...
    async def add_currency(self, interaction: discord.Interaction, player_id: str, amount: int, currency: str):
        try:
            from services.economy.economy_service import add_currency
            await add_currency(interaction, player_id, amount, currency)
        except ImportError:
    ...
    async def remove_currency(self, interaction: discord.Interaction, player_id: str, amount: int, currency: str):
        try:
            from services.economy.economy_service import remove_currency
            await remove_currency(interaction, player_id, amount, currency)
        except ImportError:
    ...
    async def disable_buttons(self, interaction: discord.Interaction):
        try:
            from services.admin.admin_service import disable_buttons
            await disable_buttons(interaction)
        except ImportError:
            await interaction.response.send_message("تم إغلاق الأزرار للجلسة القديمة.", ephemeral=True)
